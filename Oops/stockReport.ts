...
class stockk {
    ...
    SharePrice: number
    totalprice: number
    constructor(StockNames: string, NumberofShare: number, SharePrice: number, totalprice: number) {
...
function user() {
    try {
        const stock = parseInt( readline.question('how many stocks you want'))
...
function Store(stock:number) {
    try {
        ...
        while (c < stock) {
            ...
            const NumberofShare = parseInt( readline.question('enter the numberof shares'))
            if (nameRestriction.test(String(NumberofShare)) == true || NumberofShare < 0) {
                console.log("invalid plz enter the digits  ")
                throw Error
            }
            const SharePrice = parseInt( readline.question('enter the share price '))
